...
lazy_static! {
    static ref HEADER_WITH_WIRE_TO_SURFACE_1_BUFFER: Vec<u8> = {
        [
            &WIRE_TO_SURFACE_1_HEADER_BUFFER[..],
    ...
    static ref HEADER_WITH_FRAME_ACKNOWLEDGE_BUFFER: Vec<u8> = {
        [
            &FRAME_ACKNOWLEDGE_HEADER_BUFFER[..],
            &FRAME_ACKNOWLEDGE_BUFFER[..],
        ]
        .concat()
    };
    static ref HEADER_WITH_WIRE_TO_SURFACE_1: ServerPdu =
        ServerPdu::WireToSurface1(WIRE_TO_SURFACE_1.clone());
    static ref HEADER_WITH_FRAME_ACKNOWLEDGE: ClientPdu =
        ClientPdu::FrameAcknowledge(FRAME_ACKNOWLEDGE.clone());
}
...
fn from_buffer_correctly_parses_client_pdu() {
    ...
    assert_eq!(
        *HEADER_WITH_FRAME_ACKNOWLEDGE,
        ClientPdu::from_buffer(buffer).unwrap()
    );
}
