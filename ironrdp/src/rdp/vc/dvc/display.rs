...
impl PduParsing for Monitor {
    ...
    fn from_buffer(mut stream: impl Read) -> Result<Self, Self::Error> {
        let flags = MonitorFlags::from_bits(stream.read_u32::<LittleEndian>()?).ok_or(
            io::Error::new(io::ErrorKind::InvalidData, "Invalid monitor flags"),
        )?;
        ...
        let orientation = Orientation::from_u32(stream.read_u32::<LittleEndian>()?).ok_or(
            io::Error::new(io::ErrorKind::InvalidData, "Invalid monitor orientation"),
        )?;
...
impl PduParsing for ServerPdu {
    ...
    fn from_buffer(mut stream: impl io::Read) -> Result<Self, Self::Error> {
        let pdu_type = ServerPduType::from_u32(stream.read_u32::<LittleEndian>()?)
            .ok_or(DisplayPipelineError::InvalidCmdId)?;
    ...
    fn to_buffer(&self, mut stream: impl io::Write) -> Result<(), Self::Error> {
        ...
        match self {
            ServerPdu::DisplayControlCaps(pdu) => pdu
                .to_buffer(&mut stream)
                .map_err(DisplayPipelineError::from),
        }
    }
...
impl PduParsing for ClientPdu {
    ...
    fn from_buffer(mut stream: impl io::Read) -> Result<Self, Self::Error> {
        let pdu_type = ClientPduType::from_u32(stream.read_u32::<LittleEndian>()?)
            .ok_or(DisplayPipelineError::InvalidCmdId)?;
        let pdu_length = stream.read_u32::<LittleEndian>()? as usize;
    ...
    fn to_buffer(&self, mut stream: impl io::Write) -> Result<(), Self::Error> {
        ...
        match self {
            ClientPdu::DisplayControlMonitorLayout(pdu) => pdu
                .to_buffer(&mut stream)
                .map_err(DisplayPipelineError::from),
        }
    }
...
impl_from_error!(
    io::Error,
    DisplayPipelineError,
    DisplayPipelineError::IOError
);
