...
use ironrdp_session::{DvcHandlerFactory, GfxHandler, ErasedWriter};
...
impl GfxHandler for MessagePassingGfxHandler {
    fn on_message(&self, message: ServerPdu) -> Result<Option<ironrdp::dvc::gfx::ClientPdu>, RdpError> {
        self.channel.send(message).map_err(|e| RdpError::SendError(e.to_string()))?;
        Ok(None)
    }
}
...
) -> Result<(), RdpError>
{
    ...
    tokio::spawn(async move {
        handle_input_events(receiver, stream).await
    });
