...
pub fn handle_input_events<T>(receiver: Receiver<FastPathInputEvent>, mut event_stream: T)
where
    T: io::Write + 'static + Clone + Send,
{
    loop {
        ...
        let _result = event_stream.write_all(data.as_slice());
        let _result = event_stream.flush();
    }
}
